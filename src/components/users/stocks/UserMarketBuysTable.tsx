...
export const UserMarketBuysTable = () => {
    const record = useRecordContext();
    const isSmall = useMediaQuery<Theme>((theme) => theme.breakpoints.down('sm'));

    const { data, isLoading, error } = useGetList('market_buyer', {
        filter: { user_id: record?.ID }
    });

    if (isLoading) return <CircularProgress />;
    if (error) return <Typography color="error">Ошибка загрузки покупок</Typography>;
    
     if (!data || data.length === 0) 
        return (<div >
                <Typography variant="h6" gutterBottom>Покупки акций на рынке</Typography> 
